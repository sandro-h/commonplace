...
export function cleanDoneMoments(content: string, parseConfig?: ParseConfig): Edit[] {
    ...
    const todos = parseMomentsLines(new ExistingLineIterator(lines), parseConfig ? parseConfig : createParseConfig())
...
export function trashDoneMoments(content: string, parseConfig?: ParseConfig, fixedTime?: Date | null): [Edit[], Edit[]] {
    ...
    const trashTime = fixedTime ? fixedTime : new Date()
    const trashHeader = `\
------------------
...
`
    ...
    trashEdits.forEach(e => (e as InsertEdit).offset = EOF_OFFSET)
